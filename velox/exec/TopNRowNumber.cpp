...
char* TopNRowNumber::processRowBeforeLimit(
    vector_size_t /*index*/,
    TopRows& partition) {
...
char* TopNRowNumber::processRowBeyondLimit(
    vector_size_t /*index*/,
    TopRows& partition) {
...
void TopNRowNumber::processInputRow(vector_size_t index, TopRows& partition) {
  ...
  if (partition.topRank < limit_) {
    newRow = processRowBeforeLimit(index, partition);
  } else {
    char* topRow = topRows.top();
    ...
    if (result > 0) {
      newRow = processRowBeyondLimit(index, partition);
    }
  }
